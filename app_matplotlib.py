...
if st.sidebar.button("Carica dati e mostra plot"):
    ...
    #concatenate unisce i punti in segmenti, creando un array di segmenti 2D, dove ogni segmento è rappresentato da due punti consecutivi.ù
    ...
    # Punti di inizio frenata con alone bianco (triangolo verso il basso)
    ax.scatter(brake_pts['X'], brake_pts['Y'],
            marker='v', c='white', s=120, label='Frenata', edgecolors='black', linewidths=1.5, zorder=3)
    
    # Punti di fine frenata con alone bianco (triangolo verso il basso)
    ax.scatter(release_pts['X'], release_pts['Y'],
            marker='^', c='white', s=120, label='Rilascio freno', edgecolors='black', linewidths=1.5, zorder=3)
